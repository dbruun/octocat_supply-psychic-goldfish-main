...
Focus on correctness, security, data integrity, and consistency in the Spring Boot + SQLite persistence layer.
...
- Preserve consistent naming: camelCase in Java models, snake_case in SQL columns with explicit mappings.
...
- For route additions, integration test hitting the real Spring Boot app (MockMvc or TestRestTemplate) verifying status + response shape.
