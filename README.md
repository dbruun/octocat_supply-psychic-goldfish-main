# 🚀 OctoCAT Supply: The Ultimate GitHub Copilot Demo v4.1.1
...
- **Backend**: Spring Boot, Java, SQLite, OpenAPI/Swagger
...
```bash
cd api && ./mvnw spring-boot:run -Dspring-boot.run.arguments="--init-db" -Dmaven.test.skip=true
cd api && ./mvnw spring-boot:run -Dspring-boot.run.arguments="--init-db --seed-db" -Dmaven.test.skip=true
```
